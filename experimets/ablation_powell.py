    ...
    for qt in qtypes:
        for bits in [2, 3, 4]:
            run(["python", "quantization/posttraining/layer_scale_optimization.py"] + ['-a', mset['model']] + ['-b', str(mset['bs'])]
                ...
                + ['-ba', str(bits)] + ['--qtype', qt] + "--min_method Powell -maxi 2 --init_method dynamic -cs 128".split(" ")
                )
        ...
        for bits in [3, 4]:
            run(["python", "quantization/posttraining/layer_scale_optimization.py"] + ['-a', mset['model']] + ['-b', str(
                ...
                + ['--dataset', 'imagenet'] + ['--gpu_ids'] + " ".join(map(str, mset['dev'])).split(" ")
                + "--pretrained --custom_resnet".split(" ") + ['-exp', exp_name]
                + ['-bw', str(bits)] + ['--qtype', qt] + "--min_method Powell -maxi 2 --init_method dynamic -cs 128".split(
                " ")
                )
        ...
        for bits in [4]:
            run(["python", "quantization/posttraining/layer_scale_optimization.py"] + ['-a', mset['model']] + ['-b', str(
                ...
                + ['--dataset', 'imagenet'] + ['--gpu_ids'] + " ".join(map(str, mset['dev'])).split(" ")
                + "--pretrained --custom_resnet".split(" ") + ['-exp', exp_name]
                + ['-ba', str(bits)] + ['-bw', str(bits)] + ['--qtype', qt] + "--min_method Powell -maxi 2 --init_method dynamic -cs 128".split(
                " ")
                )
